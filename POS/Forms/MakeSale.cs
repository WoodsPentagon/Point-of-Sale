    ...
    public partial class MakeSale : Form
    {
        ...
        private void StockinForm_Load(object sender, EventArgs e)
        {
            ...
            using (var p = new POSEntities())
            {
                ...
                var inventoryItems = p.Products.Select(x => x.Item.Name).ToArray();
        ...
        private void searchBtn_Click(object sender, EventArgs e)
        {
            ...
            using (var p = new POSEntities())
            {
                var items = p.Products.Where(x => x.Item.Barcode == searchText.Text);

                if (items.Count() == 0)
                {
                    items = p.Products.Where(x => x.SerialNumber == searchText.Text);
                    if (items.Count() == 0)
                    {
                        items = p.Products.Where(x => x.Item.Name.Contains(searchText.Text));
                    }
                }
                var filtered = items.ToArray().Where(x => !inCart.Any(y => y.Barcode == x.Item.Barcode && y.Serial == x.SerialNumber && y.Supplier == x.Supplier.Name && y.Quantity >= x.Quantity));
                ...
                foreach (var i in filtered)
                {
                    ...
                    var j = inCart.FirstOrDefault(x => inCart.Any(y => y.Barcode == i.Item.Barcode && y.Serial == i.SerialNumber && y.Supplier == i.Supplier.Name));
                    int newQuant = i.Quantity - (j == null ? 0 : j.Quantity);

                    itemsTable.Rows.Add(i.Item.Barcode, i.SerialNumber, i.Item.Name, i.Quantity == 0 ? "Infinite" : newQuant.ToString(), i.Item.SellingPrice, i.Supplier?.Name);
                }
            }
        }
